...
const fetchPaymentStatus = async (merchantTxId) => {
    ...
    try {
        const response = await fetch(`https://merchant-api.acq.${bluecodeDomain}/v4/status/?merchant_tx_id=${merchantTxId}`, {
            method: "POST",
            headers: {
                "Authorization": "Basic " + btoa(`${merchantAccess}:${merchantSecret}`),
...
generateButton.addEventListener('click', async () => {
    ...
    try {
        ...
        const response = await fetch(`https://merchant-api.acq.${bluecodeDomain}/v4/register`, {
            method: "POST",
            headers: {
                "Authorization": "Basic " + btoa(`${merchantAccess}:${merchantSecret}`),
                "Content-Type": "application/json"
