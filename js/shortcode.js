...
function addNumber(num) {
    amount += num;
    document.getElementById('amount').value = 'NGN ' + (parseFloat(amount) / 100).toFixed(2);
}
...
async function processPayment() {
    ...
    try {
        const response = await fetch('https://merchant-api.acq.int.bluecode.ng/v4/payment', {
            method: 'POST',
            headers: {
                'Authorization': 'Basic ' + btoa('TEAM_RENISIDE:061f3831-34bb-43b4-8d7e-c70b3b087a42'),
        ...
        if (response.ok) {
            ...
            document.getElementById('payment-status').innerText = `Payment Status: ${result.result}`;
...
async function checkPaymentStatus(merchantTxId) {
    try {
        const response = await fetch(`https://merchant-api.acq.int.bluecode.ng/v4/status/?merchant_tx_id=${merchantTxId}`, {
            method: 'POST',
            headers: {
                'Authorization': 'Basic ' + btoa('TEAM_RENISIDE:061f3831-34bb-43b4-8d7e-c70b3b087a42'),
                'Content-Type': 'application/json'
        ...
        if (response.ok) {
            const result = await response.json();
            console.log('Payment Status:', result);
            document.getElementById('payment-status').innerText = `Payment Status: ${result.payment.state}`;
            if (result.payment.state === 'APPROVED' || result.payment.state === 'DECLINED') {
                clearInterval(statusInterval); // Stop polling if payment is approved or declined
            }
        } else {
            document.getElementById('payment-status').innerText = 'Failed to fetch payment status.';
