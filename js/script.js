    ...
    try {
        const response = await fetch(url, {
            ...
            headers: {
                ...
                'Authorization': 'Basic ' + btoa('fc1f9f72-02c9-47e2-99ea-bc33ba2906d4:da1616ef-caad-4c7b-b7af-e9b98fb97960')
            },
            body: JSON.stringify({
                merchant: {
                    fees: {
...
async function createBranch(merchantExtId, branchData) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('generatedBranchExtId').textContent = generatedBranchExtId;
            document.getElementById('generatedMerchantBranchId').textContent = generatedMerchantBranchId;
...
document.getElementById('fetchMerchantForm')?.addEventListener('submit', async function (event) {
    event.preventDefault();
    const merchantExtId = document.getElementById('merchant_ext_id').value;
    ...
    if (merchantExtId) {
        ...
        if (merchantDetails) {
            ...
            alert('Failed to fetch merchant details. Please check the external ID and try again.');
        }
    } else {
...
document.getElementById('fetchBranchForm')?.addEventListener('submit', async function (event) {
    event.preventDefault();
    const merchantExtId = document.getElementById('merchant_ext_id').value;
    ...
    if (merchantExtId && branchExtId) {
        ...
        if (branchDetails) {
            ...
            alert('Failed to fetch branch details. Please check the external IDs and try again.');
        }
    } else {
...
function displayMerchantDetails(merchantDetails) {
    ...
    document.getElementById('merchantAddress').textContent = `${merchantData.address.line_1}, ${merchantData.address.city}, ${merchantData.address.country}, ${merchantData.address.zip}`;
    document.getElementById('contactName').textContent = merchantData.contact.name;
    document.getElementById('contactEmail').textContent = merchantData.contact.emails.join(', ');
    document.getElementById('contactPhone').textContent = merchantData.contact.phone;
    document.getElementById('categoryCode').textContent = merchantData.category_code;
    document.getElementById('extId').textContent = merchantData.ext_id;
    document.getElementById('billingPeriod').textContent = merchantData.billing.period;
    document.getElementById('transactionSettings').textContent = JSON.stringify(merchantData.transaction_settings, null, 2);
...
function displayBranchDetails(branchDetails) {
    ...
    document.getElementById('branchAddress').textContent = `${branchData.address.line_1}, ${branchData.address.city}, ${branchData.address.country}`;
    document.getElementById('contactName').textContent = branchData.contact.name;
    document.getElementById('contactEmail').textContent = branchData.contact.emails.join(', ');
    document.getElementById('contactPhone').textContent = branchData.contact.phone;
    document.getElementById('merchantExtId').textContent = branchData.merchant_ext_id;
...
document.getElementById('updateBranchForm')?.addEventListener('submit', async function (event) {
    event.preventDefault();
    ...
    const branchData = {
        ...
        booking_reference_prefix: document.getElementById('booking_reference_prefix').value,
        state: document.getElementById('state').value,
        virtual_terminal: JSON.parse(document.getElementById('virtual_terminal').value)
    };
